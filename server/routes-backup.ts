...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const filters = {
        ...
        departureDate: req.query.departureDate
          ? new Date(req.query.departureDate as string)
          : undefined,
        returnDate: req.query.returnDate ? new Date(req.query.returnDate as string) : undefined,
        minPrice: req.query.minPrice ? parseFloat(req.query.minPrice as string) : undefined,
        maxPrice: req.query.maxPrice ? parseFloat(req.query.maxPrice as string) : undefined,
        duration: req.query.duration
          ? (req.query.duration as string).split(',').map((d) => parseInt(d))
          : undefined,
        cruiseLines: req.query.cruiseLines
          ? (req.query.cruiseLines as string).split(',')
          : undefined,
  ...
  app.post('/api/cabins/hold', async (req, res) => {
    try {
      ...
      const hold = await storage.createCabinHold(
        cruiseId,
        cabinTypeId,
        quantity,
        userId,
