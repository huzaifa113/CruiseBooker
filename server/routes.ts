...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/notify-payment', async (req, res) => {
    try {
      ...
      if (!amount || !created_at || !status || !user_email) {
        return res
          .status(400)
          .json({ message: 'Missing required fields: amount, created_at, status, user_email' });
      }

      console.log(
        `Sending payment status email to ${user_email} for amount $${amount}, status: ${status}`
      );
      const success = await sendPaymentStatusEmail(
        user_email,
        amount,
        created_at,
        status,
  ...
  app.post('/api/notify-booking', async (req, res) => {
    try {
      ...
      if (!booking_details || !status || !user_email) {
        return res
          .status(400)
          .json({ message: 'Missing required fields: booking_details, status, user_email' });
      }

      console.log(
        `Sending booking status email to ${user_email} for booking ${booking_details.confirmationNumber}, status: ${status}`
      );
  ...
  app.get('/api/cruises', async (req, res) => {
    try {
      ...
      // Simplified search: Only parse destination parameter
      if (req.query.destination) filters.destination = req.query.destination as string;

      // NOTE: Other search parameters (dates, guest count) are ignored for simplicity
  ...
  app.post('/api/bookings', optionalAuth, async (req, res) => {
    try {
      ...
      if (!canBook) {
        return res
          .status(400)
          .json({ message: 'Selected cabin type is not available for the requested dates' });
      }
      ...
      try {
        ...
        if (bookingDetails?.primaryGuestEmail) {
          console.log(`Sending booking creation email to ${bookingDetails.primaryGuestEmail} for booking ${booking.confirmationNumber}`);
          const success = await sendBookingStatusEmail(bookingDetails.primaryGuestEmail, bookingDetails, 'created');
          if (success) {
            console.log(`Booking creation email sent for booking: ${booking.confirmationNumber}`);
          } else {
            console.log(`Failed to send booking creation email for booking: ${booking.confirmationNumber}`);
          }
        }
      } catch (error) {
  ...
  app.post('/api/promotions/check-eligibility', async (req, res) => {
    try {
      ...
      if (!bookingAmount || !promotionIds || !bookingData) {
        return res
          .status(400)
          .json({ message: 'bookingAmount, promotionIds, and bookingData are required' });
      }

      const eligibilityResult = await storage.checkPromotionEligibility(
        bookingAmount,
        promotionIds,
        bookingData
      );
  ...
  app.get('/api/bookings/:id/invoice-pdf', async (req, res) => {
    try {
      ...
      if (pdfBuffer) {
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="invoice-${booking.confirmationNumber}.pdf"`
        );
        res.send(pdfBuffer);
      } else {
        ...
        return res
          .status(404)
          .json({ message: 'PDF generation not available, using print fallback' });
      }
    } catch (error: any) {
  ...
  app.get('/api/bookings/:id/calendar', async (req, res) => {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="cruise-itinerary-${booking.confirmationNumber}.ics"`
      );
    ...
    try {
      ...
      try {
        if (bookingWithDetails?.primaryGuestEmail) {
          console.log(`Sending payment status email to ${bookingWithDetails.primaryGuestEmail} for amount $${amount}, status: paid`);
          const success = await sendPaymentStatusEmail(
            bookingWithDetails.primaryGuestEmail,
            amount.toString(),
            new Date().toISOString(),
            'paid',
            bookingWithDetails.confirmationNumber
          );
          if (success) {
            console.log(`Payment confirmation email sent for booking: ${bookingWithDetails?.confirmationNumber}`);
          } else {
            console.log(`Failed to send payment confirmation email for booking: ${bookingWithDetails?.confirmationNumber}`);
          }
        }
      } catch (error) {
      ...
      try {
        if (bookingWithDetails?.primaryGuestEmail) {
          console.log(`Sending booking status email to ${bookingWithDetails.primaryGuestEmail} for booking ${bookingWithDetails.confirmationNumber}, status: confirmed`);
          const success = await sendBookingStatusEmail(bookingWithDetails.primaryGuestEmail, bookingWithDetails, 'confirmed');
          if (success) {
            console.log(`Booking confirmation email sent for booking: ${bookingWithDetails?.confirmationNumber}`);
          } else {
            console.log(`Failed to send booking confirmation email for booking: ${bookingWithDetails?.confirmationNumber}`);
          }
        }
      } catch (error) {
  ...
  app.post('/api/admin/promotions', async (req, res) => {
    try {
      console.log('📝 SERVER: Received promotion creation request');
      ...
      console.log('🔍 SERVER: Extracted conditions:', conditions);
      console.log('🎯 SERVER: Conditions type:', typeof conditions);
      ...
      console.log(
        '📦 SERVER: Final promotion data to save:',
        JSON.stringify(promotionData, null, 2)
      );
      console.log('🎯 SERVER: Final conditions:', promotionData.conditions);
